...
CREATE OR REPLACE FUNCTION mmi_master.make_view_road_network(
	stat_code text,
	sch_nme text,
	view_sch text)
    RETURNS integer
	...
	BEGIN
		If UPPER(stat_code) = 'ALL' Then
			...
			Loop
				...
				k=k+1;
				EXECUTE 'SELECT count(table_name) FROM information_schema.tables WHERE UPPER(table_name) LIKE '''||UPPER(stat_code)||'____ROAD_NETWORK'' AND TABLE_SCHEMA ='''||sch_nme||'''' into count;
				i=0;
				j=0;
			IF count > 1 THEN
				Raise Info 'Create View for State %',stat_code;
				-- tbl_nme_road=''|| UPPER(stat_code) ||'_ROAD_NETWORK';
				FOR r IN EXECUTE FORMAT('SELECT table_name FROM information_schema.tables WHERE UPPER(table_name) LIKE '''||UPPER(stat_code)||'____ROAD_NETWORK'' AND TABLE_SCHEMA ='''||sch_nme||''' ') 
				LOOP
				...
				END LOOP;			
				EXECUTE 'drop VIEW if exists '||view_sch||'."'|| UPPER(stat_code) ||'_ROAD_NETWORK"';
				EXECUTE 'CREATE VIEW '||view_sch||'."'|| UPPER(stat_code) ||'_ROAD_NETWORK" As ('|| conquery||')';
				RETURNVAL = 1;
			END IF;
			End Loop;
		Else	
			...
			IF count > 1 THEN
				...
				END LOOP;				
				EXECUTE'drop VIEW if exists '||view_sch||'."'|| UPPER(stat_code)||'_ROAD_NETWORK"';
				EXECUTE'CREATE VIEW '||view_sch||'."'|| UPPER(stat_code) ||'_ROAD_NETWORK" As 
						('||conquery||')';
				RETURNVAL = 1;
			END IF;
